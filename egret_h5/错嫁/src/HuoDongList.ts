 ...
 *
 */
class HuoDongList extends egret.DisplayObjectContainer {
    private gameimgSheet: egret.SpriteSheet;
    private huodongscene: HuoDongScene;
    private netloading: NetLoadingUI;
    private duihuanlayer: egret.Sprite;
    private coverimgSheet: egret.SpriteSheet;
    private huodongimgSheet: egret.SpriteSheet;
    private text_duihuan: egret.TextField;
    private gamescene: GameScene;
    private guanzhu_index: number;
    private toastBgLayer: egret.Sprite;
    private toastLayer: egret.Sprite;
    private test_text: egret.TextField;
    public constructor(thisObj: any, gamescene: GameScene) {
        super();
        this.gameimgSheet = RES.getRes("gameimg_json");
        this.coverimgSheet = RES.getRes("coverimg_json");
        this.huodongimgSheet = RES.getRes("huodong_json");
        this.huodongscene = thisObj;
    ...
    private init() {
        ...
        if (!GameUtils.noqqQun) {
            var qqGroupNum: egret.TextField = new egret.TextField();
    ...
    private fenxiang(evt: egret.TouchEvent) {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private qianDao(evt: egret.TouchEvent) {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private guanZhu(evt: egret.TouchEvent) {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private callBackHuoDongFollow(event: EventData) {
    ...
    private duiHuan(evt: egret.TouchEvent) {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private drawDuiHuanKuang(): void {
        ...
        var duihuankuang1: egret.Bitmap = new egret.Bitmap();
        ...
        var sendduihuan: egret.Bitmap = new egret.Bitmap();
        ...
        var closeduihuan: egret.Bitmap = new egret.Bitmap();
    ...
    private sendDuiHuan(evt: egret.TouchEvent): void {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private getDuiHuanComplete(event: egret.Event) {
    ...
    private closeDuiHuan(evt: egret.TouchEvent): void {
        var dianeff = new DianEff(this.huodongscene, evt.stageX, evt.stageY);
        if (GameUtils.gameSandPause) {
            return;
        }
    ...
    private getGuanZhuComplete(event: egret.Event) {
        ...
        if (obj.result == 1) {
            var guanzhubtn: egret.DisplayObject = this.getChildByName("guanzhuopen");
            if (guanzhubtn) {
    ...
    private getQianDaoComplete(event: egret.Event) {
        var obj = NetWorkUtils.getResponseObj("p_204.k", event);
        if (obj.info) {
            var qiandaobtn: egret.DisplayObject = this.getChildByName("qiandaoopen");
            if (qiandaobtn) {
