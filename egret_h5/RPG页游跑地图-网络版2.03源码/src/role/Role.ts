...
class Role extends egret.Sprite {
    ...
    private timer:egret.Timer = new egret.Timer(1000/20);
    public step:number = 0;//动作的标识
    private step2:number = 0;
    private i:number = 0;//起始帧
    private n:number = 0;//帧数
    private kaiguan:boolean = true;
    public playerPool:string;//池标识，用于 换装
    //对应各个动作的 名字 补充
    private m_josn:string = "";
    private m_texture:string = "";
    private m_name:string = "";

    private is_attack:boolean = false;
    ...
    public startRole(playerPool:string,j_josn:string,t_texture:string, n_name:string,stepA:number): egret.MovieClip {
    ...
    private onRoleTimer(evt:egret.Event):void
    {
        ...
        if(this.step2 != this.step){
            ...
            /*if(this.kaiguan){
                this.mc.gotoAndPlay(this.i);
                this.kaiguan = false;//首次运行 开启 播放剪辑
            }*/
        }
