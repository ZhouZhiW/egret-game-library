...
var GameLayer = (function (_super) {
    ...
    function GameLayer() {
        ...
        /**现在的格子*/
        this.onGrid = new egret.Point();
        /**要到的格子*/
        this.inGrid = new egret.Point();
        this.tile_a = new Tile();
    ...
    p.onSceneEvent = function (e) {
        ...
        if (e.type == "mapLoadOk") {
            ...
            //获取人物的坐标
            var xx = new egret.Point();
            xx.x = RoleConstant.player.x;
            xx.y = RoleConstant.player.y;
            var hh = this.mapPoint.Point_b(xx.x, xx.y);
    ...
    p.onTouch = function (e) {
        ...
        //this.hero.reMoveTw();
        var hh = GameLayer.gameScene.reMoveTw();
        ...
        if (this.onGrid.x == this.inGrid.x && this.onGrid.y == this.inGrid.y) {
            ...
            var arr = this.aStar_a.find(this.onGrid.x, this.onGrid.y, this.inGrid.x, this.inGrid.y);
            var grid_z = [];
            var ar_t = "";
            if (arr) {
    ...
    /**移动到点*/
    p.movePoint = function (zx, zy) {
        //  获取屏幕大小
        this.stageW = egret.MainContext.instance.stage.stageWidth;
        this.stageH = egret.MainContext.instance.stage.stageHeight;
        var sW = this.stageW / 2;
        var sH = this.stageH / 2;
        //计算用户点击 与 中心点 的距离
        var downX = Math.abs(zx - sW);
        var downY = Math.abs(zy - sH);
        if (downX && downY) {
            this.speedX = downX;
            this.speedY = downY;
        }
        ...
        this.downPoint.y = zy;
        //计算角度、方向
        var angleSpeed = Math.atan2(this.downPoint.y - sH, this.downPoint.x - sW);
        var vr = angleSpeed * 180 / Math.PI; //角度
        var n = this.FangXiang(vr); //方向
