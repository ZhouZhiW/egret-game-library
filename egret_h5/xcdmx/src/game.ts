class game extends egret.Sprite {
    ...
    // private Block_Array: Array<any> = [];
    private _GroupRect: GroupRect;
    private main_map: Map;
    private glodBg_Array: Array<any> = [];
    private ISCANDOWN: boolean = false;
    private move_Y = 200
    ...
    private onClick_This_Begin(evt: egret.TouchEvent) {/*点击屏幕回调*///evt.currentTarget表示绑定的对象
        var click_index = 0
    ...
    private click_num: number;//表示的是点击方块组中有多少小方块（用于计算分数的时候使用）
    private onClick_This_Move(evt: egret.TouchEvent) {/*移动鼠标回调*///evt.currentTarget表示绑定的对象
        if (this._GroupRect.ISCLICK) {
            ...
            if (evt.stageX >= min_x && evt.stageX <= max_x && evt.stageY - this.move_Y > min_y && evt.stageY - this.move_Y < max_y) {
                var len_num = 0;
                for (var a = 0; a < this._GroupRect._group[this._GroupRect.getid]._rect.length; a++) {//判断所有方块都处于棋盘位置
                ...
                if (len_num == this._GroupRect._group[this._GroupRect.getid]._rect.length) {
                    ...
                    } else {//解决当鼠标先移动到空白棋盘处 又移到有方块的格子时提示框没有消失。
                        this.remove_glodBg_Array()//移除背景提示框
                    }
                } else {//当鼠标移动到棋盘上时有移除棋盘时棋盘上有创建的提示框所以要删除他。
                    this.remove_glodBg_Array()//移除背景提示框
                }
            } else {
                this.remove_glodBg_Array()//移除背景提示框
    ...
    private onClick_This_End(evt: egret.TouchEvent) {/*鼠标点击结束回调*/
        if (this._GroupRect.ISCLICK) {
            ...
            if (this._GroupRect._group[this._GroupRect.getid]._rect.length == this.glodBg_Array.length) {
                ...
                var _savedata = [];
                var _saveDowndata = [];
                for (var i = 0; i < data.Block_Array.length; i++) {//保存上面待消除的方块
    ...
    private SaveData(_upData, _downData, _value, _Maxvalue, _score, _bestScore, _update_num, _rotation_num, _level) {
        if (_upData) {
    ...
    private CreateNewGroupRect() {/*当下面三个已经全部移动棋盘上时，会调此方法进行创建新的数组方块*/
        data.CLICKNUM = 3;
    ...
    private onClick_This_Cancel() {/*当点击取消时*/
        if (this._GroupRect.ISCLICK) {
            this._GroupRect._group[this._GroupRect.getid].scaleX = 0.5;
            this._GroupRect._group[this._GroupRect.getid].scaleY = 0.5;
            this._GroupRect._group[this._GroupRect.getid].x = data.DOWN_POSTION[this._GroupRect.getid][0]
    ...
    private checkRect() {//检测是否有满足一整行或者一整列
        var _needRemoveRectRow: Array<any> = [];
        var _needRemoveRectCol: Array<any> = [];
        //先从每行判断
        for (var a = 0; a < data.map_row; a++) {
            ...
            if (index_row == 0) {
                var needRemoveRect_row: Array<any> = [];
        ...
        for (var a = 0; a < data.map_col; a++) {
            ...
            if (index_col == 0) {
                var needRemoveRect_col: Array<any> = [];
    ...
    private deleteRect(_Array: Array<any>) {//对满足一整行或者一整列的进行删除操作
    ...
    private remove_glodBg_Array() {//移除方块的提示框
    ...
    private CreateAnction(x: number, y: number, _rotation: number) { /*添加动画*/
        ...
        var mc1: egret.MovieClip = new egret.MovieClip(mcFactory.generateMovieClipData("test"));
        this.addChild(mc1);
        mc1.play(1);
    ...
    private addBtn() {/*创建按钮*/
        ...
        var btn1: egret.Bitmap = this.createBitmapByName("refresh_png");
        ...
        var btn2: egret.Bitmap = this.createBitmapByName("transform_png");
    ...
    private createGoldSprite(x: number, y: number) {/*创建方块的提示框*/
        var sp: egret.Bitmap = this.createBitmapByName("geizibg_png");
    ...
    private Score_text: egret.TextField;
    private BestScore_text: egret.TextField;
    public _updatedata: egret.TextField;
    public _rotationdata: egret.TextField;;
    private addScorePanel() {/*添加分数面板*/
    ...
    private CreateTimerTiao() {
        var egret_width = egret.MainContext.instance.stage.stageWidth;
        var sp_height = 180;
        var btn2: egret.Bitmap = this.createBitmapByName("bar_empty_png");
        ...
        var btn: egret.Bitmap = this.createBitmapByName("bar_leaf_png");
        btn.x = egret_width / 2;
        btn.y = sp_height;
    ...
    private timerHandler(value: number) {
